...
1. 【Content-Length更新】一定要去掉勾选
   ...
   ![pipline1](F:\file\知识\渗透测试\pipline1.png)
...
4. 使用双请求包进⾏请求，响应也是两次，大多数的WAF仅检测第一个的请求，利用二次请求绕过WAF的检测。
   ...
   ![pipline4](F:\file\知识\渗透测试\pipline2.png)
...
1. 原始数据是【id=1‘UniOn sELeCt 1,user()%23】进行测试 ，可以得知已经被安全狗拦截。
   ...
   ![Block_transmission1](F:\file\知识\渗透测试\Block_transmission1.png)
...
3. 一般的WAF都会有检测，可以通过分块传输进行绕过，经过分块转换之后，可将数据分为多个块。回显可以得知已经成功绕过安全狗获取到user账号
   ...
   ![Block_transmission2](F:\file\知识\渗透测试\Block_transmission2.png)
...
5. 对内容进行编码，使用方法为在Content-Type类型后加入【charset=ibm500】即可。
   ...
   ![HTTP_protocol_coverage1](F:\file\知识\渗透测试\HTTP_protocol_coverage1.png)

6. 组合绕过，配合分块传输的方式进行绕过

   ![HTTP_protocol_coverage2](F:\file\知识\渗透测试\HTTP_protocol_coverage2.png)
...
![passwaf1](F:\file\知识\渗透测试\passwaf1.png)
...
![passwaf2](F:\file\知识\渗透测试\passwaf2.png)
